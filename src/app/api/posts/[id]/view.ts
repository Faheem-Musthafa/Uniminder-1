// POST /api/posts/[id]/view - Register a post view
  ...
  try {
    ...
    if (!existing) {
      ...
      const { error: insError } = await getSupabase()
        .from('post_interactions')
        .insert({ post_id: postId, user_id: profile.id, interaction_type: 'view' });
      if (insError) throw AppErrors.internal(insError.message);
    }

    // Increment views_count using DB function
    await getSupabase().rpc('increment_post_views', { post_id: postId });
